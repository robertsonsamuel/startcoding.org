...
userSchema.statics.login = function (userInfo, cb) {
  ...
  User.findOne({ username: userInfo.username }).select('+password').exec(function (err, foundUser) {
...
userSchema.statics.register = function (userInfo, cb) {
  ...
  User.findOne(newUserQuery).select('+email').exec(function (err, user) {
...
userSchema.statics.reset = function (req, cb) {
  async.waterfall([function (done) {
    User.findOne({ resetPasswordToken: req.params.token, resetPasswordExpires: { $gt: Date.now() } }).select('+email').exec(function (err, user) {
